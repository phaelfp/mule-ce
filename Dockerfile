...
RUN apk update \
 ...
 && wget -O /opt/mule-standalone-3.8.0.tar.gz https://repository.mulesoft.org/nexus/content/repositories/releases/org/mule/distributions/mule-standalone/3.8.0/mule-standalone-3.8.0.tar.gz \
 && cd /opt \
 && echo "d9279b3f0373587715613341a16483f3  mule-standalone-3.8.0.tar.gz" | md5sum -c \
 && tar xvzf /opt/mule-standalone-3.8.0.tar.gz \
 && rm /opt/mule-standalone-3.8.0.tar.gz \
